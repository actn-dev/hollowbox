...
const LIQUIDITY_POOL_ID =
  "befdfc5b6b89a4e1def19e08ad964257b826657a3293c3e1f668bdb72ae8d1ea";
  ...
  const updateFromStellar = async () => {
    try {
      ...
      const response = await fetch("/api/profit-tracker/update", {
        ...
        // Send empty body to update all tracked wallets
        body: JSON.stringify({}),
      });
      ...
      // Show success message with info about updated wallets
      console.log("✅ Successfully updated profit data:", result.message);
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to update from Stellar network"
      );
    } finally {
      setIsUpdating(false);
    }
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-6 gap-4">
          <div className="text-center p-4 bg-muted/20 rounded-lg">
            ...
            <div className="text-2xl font-bold text-green-400">
              {data.combinedProfit.toFixed(2)} XLM
            </div>
          ...
          <div className="text-center p-4 bg-muted/20 rounded-lg">
            ...
            <div className="text-2xl font-bold text-primary">
              {data.combinedActionFund.toFixed(2)} XLM
            </div>
          ...
          <div className="text-center p-4 bg-muted/20 rounded-lg">
            ...
            <div className="text-2xl font-bold text-pink-400">
              {data.combinedImpactFund.toFixed(2)} XLM
            </div>
          ...
          <div className="text-center p-4 bg-muted/20 rounded-lg">
            ...
            <div className="text-2xl font-bold text-blue-400">
              {data.combinedLiquidity.toFixed(2)} XLM
            </div>
        ...
        <Alert className="border-primary/50 bg-primary/10">
          ...
          <AlertDescription className="text-white">
            ...
            to good - {data.combinedActionFund.toFixed(2)} XLM for $ACTION token
            purchases and giveaways + {data.combinedImpactFund.toFixed(2)} XLM
            for Impact Fund donations
            {data.combinedLiquidity > 0 && (
              <span>
                {" "}
                + {data.combinedLiquidity.toFixed(2)} XLM provided to
                                  ...
                                  {tx.type === "trade" ? (
                                    <div>
                                      ...
                                      <div className="text-xs text-muted-foreground">
                                        -{tx.hollowvoxAmount.toLocaleString()}{" "}
                                        HOLLOWVOX
                                      </div>
                                    </div>
                                  ) : tx.type === "liquidity" ? (
        ...
        <div className="text-xs text-muted-foreground text-center border-t border-border/50 pt-4 space-y-1">
          ...
          <p>
            ** Tracking HOLLOWVOX tokens from issuers: HOLLOWVOX-1 (...LMAX) and
            HOLLOWVOX-2 (...EJLF)
          </p>
          <p>
            *** 20% of all profits automatically allocated: 10% for $ACTION
            token purchases + 10% for Impact Fund donations + liquidity
            provision as part of Action Token commitment
          </p>
          <p>
            **** Liquidity pool tracking: XLM/HOLLOWVOX pool (
            {LIQUIDITY_POOL_ID.slice(-8)})
          </p>
          <p className="text-blue-400">
            ***** All data stored in Neon database for fast, reliable access
          </p>
        </div>
      </CardContent>
    </Card>
