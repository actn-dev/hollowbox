...
export const monitoringStore = {
  ...
  async addSuspiciousLog(
    log: Omit<SuspiciousLog, "received_at" | "id">
    ...
    const result = await db.run(sql`
      ...
      RETURNING *
    `);
    return result.rows?.[0] as unknown as SuspiciousLog;
  },
  ...
  async getSuspiciousLogs(): Promise<SuspiciousLog[]> {
    ...
    `);
    return (result.rows || []) as unknown as SuspiciousLog[];
  },

  async clearSuspiciousLogs(): Promise<void> {
    await db.run(sql`DELETE FROM suspicious_logs`);
  },
  ...
  async getWatchlist(): Promise<WatchlistEntry[]> {
    ...
    `);
    return (result.rows || []) as unknown as WatchlistEntry[];
  },
  ...
  async addWatchlistEntry(
    ...
    const result = await db.run(sql`
      ...
      RETURNING *
    `);
    return result.rows?.[0] as unknown as WatchlistEntry;
  },
  ...
  async updateWatchlistEntry(
    ...
    const result = await db.run(sql`
      ...
      RETURNING *
    `);
    return (result.rows?.[0] as unknown as WatchlistEntry) || null;
  },
  ...
  async getStats() {
    const [logsResult, watchlistResult, activeResult] = await Promise.all([
      db.run(sql`SELECT COUNT(*) as count FROM suspicious_logs`),
      db.run(sql`SELECT COUNT(*) as count FROM watchlist`),
      db.run(
    ...
    return {
      totalLogs: Number(logsResult.rows?.[0]?.count || 0),
      monitoredWallets: Number(watchlistResult.rows?.[0]?.count || 0),
      activeThreats: Number(activeResult.rows?.[0]?.count || 0),
    };
  },
};
