...
async function getUserTier(walletAddress: string): Promise<number> {
  try {
    ...
    const profitResult = await db.run(sql`
    ...
    if ((profitResult.rows || []).length > 0) {
      const balance = Number.parseFloat(String(profitResult.rows[0].balance));
    ...
    const rewardsResult = await db.run(sql`
    ...
    if ((rewardsResult.rows || []).length > 0) {
      const totalTokens = Number.parseFloat(
        String(rewardsResult.rows[0].total_tokens || "0")
      );
...
export async function GET(request: NextRequest) {
  try {
    ...
    let statsResult = await db.run(sql`
    ...
    if ((statsResult.rows || []).length === 0) {
      ...
      if (statsResult.rows[0]) statsResult.rows[0].max_scans = maxScans;
    }

    stats = statsResult.rows[0];

    // Get found signals
    const signalsResult = await db.run(sql`
    ...
    const response = {
      ...
      foundSignals: signalsResult.rows,
      userTier,
      timeUntilReset,
      canScan: (stats.scans_used ?? 0) < (stats.max_scans ?? 0),
    };
