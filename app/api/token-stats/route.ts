      ...
      try {
        ...
        if (tradesResponse.ok) {
          ...
          if (trades.length > 0) {
            ...
            if (latestTrade.price && latestTrade.price.n && latestTrade.price.d) {
              currentPrice = Number.parseFloat(latestTrade.price.n) / Number.parseFloat(latestTrade.price.d)
            }
          }
          ...
          if (tradesYesterday.length > 0 && currentPrice > 0) {
            try {
              ...
              if (yesterdayTrade.price && yesterdayTrade.price.n && yesterdayTrade.price.d) {
                const yesterdayPrice =
                  Number.parseFloat(yesterdayTrade.price.n) / Number.parseFloat(yesterdayTrade.price.d)
                if (yesterdayPrice > 0) {
                  change24h = ((currentPrice - yesterdayPrice) / yesterdayPrice) * 100
                }
              }
            } catch (error) {
              console.log(`Error calculating price change for ${issuer.name}:`, error)
            }
          }
        }
        ...
        if (supply === 0) {
          try {
            ...
            if (issuerResponse.ok) {
              ...
              if (issuerData.balances) {
                for (const balance of issuerData.balances) {
                  if (balance.asset_code === "HOLLOWVOX" && balance.asset_issuer === issuer.address) {
                    // For issued tokens, the supply is usually the total minus what the issuer holds
                    const issuerBalance = Number.parseFloat(balance.balance || "0")
            ...
            console.log(`Error fetching issuer data for ${issuer.name}:`, error)
          }
        }
          ...
          try {
            ...
            if (holdersResponse.ok) {
              ...
              if (holdersData._embedded?.records) {
                supply = holdersData._embedded.records.reduce((total: number, account: any) => {
                  if (account.balances) {
                    for (const balance of account.balances) {
                      if (balance.asset_code === "HOLLOWVOX" && balance.asset_issuer === issuer.address) {
                        return total + Number.parseFloat(balance.balance || "0")
                      }
                    }
                  }
                  return total
            ...
            console.log(`Error fetching holders data for ${issuer.name}:`, error)
          }
        }
