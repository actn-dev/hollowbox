...
export async function POST(request: NextRequest) {
  ...
  `);
    ...
    if (item.winnerAnnounced) {
      return NextResponse.json(
        { error: "Winner has already been announced for this item" },
        { status: 400 }
      );
    }

    // Get all entries for this item
    const entriesRes = await db.run(sql`
      SELECT 
        id,
    ...
    if (entries.length === 0) {
      return NextResponse.json(
        { error: "No entries found for this drawing" },
        { status: 400 }
      );
    }
    ...
    entries.forEach((entry, index) => {
      const count = Number(entry.entries || 0);
